...
class Billsform:
    def __init__(self,root,stuid):
        ...
        #Label
        head = Label(framehead, text="Bill",font=('Hack', 17 ,'bold'), fg = "black",bg = "#FFAF5E")
        head.place(x=30,y=15)
        #============ดึงข้อมูลลลลล==============

        bk_id = DB_connect.bookingid(self.stuid)
        repair = DB_connect.repair(stuid)
        if bk_id != None:

            self.billDB= DB_connect.bills(bk_id)
            if self.billDB != ():        
                dormDB=DB_connect.dormdata(bk_id) 
                userinfo = DB_connect.name(stuid) 
                ...
                # roomlbl =Label(frame4, text="Room: "+ str(roominfo[1])  , fg = "black",bg = b)
                # roomlbl.place(x=270,y=25)
                ...
                # linelbl = Label(frametext, text='-'*67 , fg = "black",bg=b, font= '7') 
                # linelbl.place(x=15,y=175)

                #frame4
                namelbl =Label(frame4, text= userinfo[1]+ ' '+ userinfo[2] , fg = "black",bg = b)
                namelbl.place(x=15,y=0)

                # floorlbl =Label(frame4, text= "Floor: " +str(roominfo[0]), fg = "black",bg = b)
                # floorlbl.place(x=200,y=25)
